...
from utils.click_method import  get_next_click3D_torch_with_gt_3Dbox
from utils.dataloaders import Dataset_Union_ALL, Union_Dataloader
...
parser.add_argument('--task_name', type=str, default='Dataset131_MICCAI_challenge_small_v2')
parser.add_argument('--dataset_id', type=int, default=130)
parser.add_argument('--click_type', type=str, default='random')
parser.add_argument('--multi_click', action='store_true', default=False)
parser.add_argument('--model_type', type=str, default='vit_b_ori')
parser.add_argument('--checkpoint', type=str, default=ROOT / 'ckpt/sam_med3d_turbo.pth')
parser.add_argument('--device', type=str, default='cuda:1')
parser.add_argument('--work_dir', type=str, default=ROOT / 'work_dir')

# train
parser.add_argument('--pcc', action='store_true', default=True)
parser.add_argument('--num_workers', type=int, default=8)
parser.add_argument('--gpu_ids', type=int, nargs='+', default=[1,3])
...
def get_dataloaders(args):
    train_dataset = Dataset_Union_ALL(paths=img_datas,dataset_id=args.dataset_id, transform=tio.Compose([
        tio.ToCanonical(),
...
class BaseTrainer:
    ...
    def init_checkpoint(self, ckp_path):
        ...
        if os.path.exists(ckp_path):
            if self.args.multi_gpu:
                dist.barrier()
                last_ckpt = torch.load(ckp_path, map_location='cpu')
            else:
                last_ckpt = torch.load(ckp_path, map_location='cpu')
    ...
    def batch_forward(self, sam_model, image_embedding, gt3D, low_res_masks,boxes=None, points=None):
    ...
    def get_points(self, prev_masks, gt3D, boxes3D):
        # import pdb; pdb.set_trace()
        boxes3D = boxes3D.squeeze(1).flatten(-2)
        batch_points, batch_labels = click_methods[self.args.click_type](prev_masks, boxes3D)
        ...
        # import pdb; pdb.set_trace()
        # print('self.click_points',torch.as_tensor(self.click_points).shape)
    ...
    def interaction(self, sam_model, image_embedding, gt3D, boxes3D, num_clicks):
        return_loss = 0
        ...
        for num_click in range(num_clicks):
            points_input, labels_input = self.get_points(prev_masks, gt3D, boxes3D)

            if num_click == random_insert or num_click == num_clicks - 1:
                low_res_masks, prev_masks = self.batch_forward(sam_model, image_embedding, gt3D, low_res_masks,boxes3D, points=None)
            else:
                low_res_masks, prev_masks = self.batch_forward(sam_model, image_embedding, gt3D, low_res_masks,boxes3D, points=[points_input, labels_input])
    ...
    def train_epoch(self, epoch, num_clicks):
        ...
        step_loss = 0
        epoch_dice = 0
        for step, (image3D, gt3D, boxes3D) in enumerate(tbar):
            ...
            with my_context():
                ...
                with amp.autocast():
                    ...
                    prev_masks, loss = self.interaction(sam_model, image_embedding, gt3D, boxes3D, num_clicks=11)                
