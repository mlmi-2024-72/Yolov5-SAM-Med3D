...
from ultralytics import YOLOv10  # 导入YOLOv10
...
def select_boxes(pred_3D_boxes, selfect_num=5):
    # 将有较多零值的 box 置为全零
    ...
    if len(non_zero_boxes) >= selfect_num:
        pred_3D_boxes = non_zero_boxes[:5]
    else:
        pred_3D_boxes = non_zero_boxes + zero_boxes[:selfect_num - len(non_zero_boxes)]

    # 如果仍不足 20 个，则补充全为 0 的 box
    while len(pred_3D_boxes) < selfect_num:
...
def pad_and_crop_with_sliding_window(img3D, boxes_3D, crop_transform, tile_step_size=0.5):
    subject = tio.Subject(
...
def run_inference(args):
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if(args.skip_existing_pred and osp.exists(pred_path)):
            ...
            sliding_window_list = pad_and_crop_with_sliding_window(image3D_full, corrected_pred_3D_boxes, crop_transform)
            # import pdb; pdb.set_trace()
            for (image3D, pos3D, mapped_boxes) in sliding_window_list:
                # import pdb; pdb.set_trace()
                ...
                for idx, seg_mask in enumerate(seg_mask_list):
                    ...
                    if seg_mask_roi.shape[-1] > gt3D_full.shape[-1]:
                        ...
                        # seg_mask_roi = F.interpolate(seg_mask_roi.unsqueeze(0).unsqueeze(1), size=(seg_mask_roi.shape[1], seg_mask_roi.shape[2], gt3D_full.shape[-1]), mode='trilinear', align_corners=False).squeeze(0).squeeze(0)
                        # seg_mask_roi = seg_mask_roi.cpu().numpy().astype(np.uint8)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task_name', type=str, default='MICCAI_Challenge_2024_use_yolov5_step_0_5')
    parser.add_argument('--save_name', type=str, default='union_out_dice_use_yolov5_step_0_5.py')
