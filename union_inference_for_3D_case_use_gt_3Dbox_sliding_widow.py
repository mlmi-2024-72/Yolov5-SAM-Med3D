...
click_methods = {
    ...
    'ritm': get_next_click3D_torch_with_gt_3Dbox,
...
def finetune_model_predict3D(img3D, gt3D, gt_3D_boxes, sam_model_tune, norm_transform, device='cuda:4', click_method='random', num_clicks=10, prev_masks=None):
    img3D = norm_transform(img3D.squeeze(dim=1)) # (N, C, W, H, D)
    img3D = img3D.unsqueeze(dim=1)
    
    gt_3D_boxes = torch.tensor(gt_3D_boxes,device=device,dtype=torch.float32)
    ...
    for click_idx in range(num_clicks):
        with torch.no_grad():
            if click_idx == 0 and torch.all(gt_3D_boxes==0):
                click_method = "no_box"
                batch_points, batch_labels = click_methods[click_method](prev_masks.to(device), img3D.to(device))
            elif click_idx == 0 and not torch.all(gt_3D_boxes==0):
                batch_points, batch_labels = click_methods[click_method](prev_masks.to(device), gt_3D_boxes)
            elif(click_idx >= 1):
                click_method = "random"
                batch_points, batch_labels = click_methods[click_method](prev_masks.to(device), gt_3D_boxes)
...
def pad_and_crop_with_sliding_window_with_box_in(img3D, boxes_3D, crop_transform, tile_step_size=0.2):
    subject = tio.Subject(
...
def pad_and_crop_with_sliding_window_with_box_overlab(img3D, boxes_3D, crop_transform, tile_step_size=0.2):
    subject = tio.Subject(
        image=tio.ScalarImage(tensor=img3D.squeeze(0))
    )
...
def run_inference(args):
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if(args.skip_existing_pred and osp.exists(pred_path)):
            ...
            # Stage2 -------------Union 3D Data and gt_3D_boxes for SAM-Med3D-------------
            ''' inference '''
            ...
            sliding_window_list = pad_and_crop_with_sliding_window_with_box_in(image3D_full, gt_3Dbox, crop_transform, tile_step_size=0.6)
            if len (sliding_window_list) == 0:
                sliding_window_list = pad_and_crop_with_sliding_window_with_box_overlab(image3D_full, gt_3Dbox, crop_transform, tile_step_size=0.6)
            ...
            for (image3D, pos3D, mapped_boxes) in tqdm(sliding_window_list, desc="Processing sliding_window"):
                ...
                for idx, seg_mask in enumerate(seg_mask_list):
                    ...
                    # seg_mask_roi = torch.tensor(seg_mask_roi, device=image3D.device, dtype=image3D.dtype)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task_name', type=str, default='MICCAI_Challenge_2024_use_gt_3Dbox_step_0_6')
    parser.add_argument('--save_name', type=str, default='./results/union_out_dice_use_gt_3Dbox_step_0_6.py')
