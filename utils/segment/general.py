...
def process_mask(protos, masks_in, bboxes, shape, ori_shape=None, upsample=False):
    """
    ...
    # ori_shape_masks = masks.clone()
    if upsample:
        masks = F.interpolate(masks[None], shape, mode="bilinear", align_corners=False)[0]  # CHW
        # if ori_shape is not None:
        #     ori_shape_masks = F.interpolate(ori_shape_masks[None], ori_shape, mode="bilinear", align_corners=False)[0]  # CHW
    return masks.gt_(0.5)
