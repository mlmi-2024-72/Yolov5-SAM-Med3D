...
def run_stage_1(model, dataloader):
    ...
    for im, slice_indexs in tqdm(dataloader, desc="Processing Slices"):
        ...
        for i, det in enumerate(pred):
            ...
            if det.shape[0] == 0 :
                ...
                save_det = torch.cat((det[:,:5], torch.tensor([[slice_indexs[i]]] * det.size(0), device=det.device)), dim=1)
...
def pad_and_crop_with_sliding_window(img3D, gt3D, boxes_3D, crop_transform, offset_mode="center"):
    subject = tio.Subject(
    ...
    for offset in offset_dict[offset_mode]:
        ...
        cropping_params = (
            max(0, oob_ori_roi[0] + padding_params[0]), max(0, vol_bound[1] - oob_ori_roi[1] + padding_params[1]),
            max(0, oob_ori_roi[2] + padding_params[2]), max(0, vol_bound[3] - oob_ori_roi[3] + padding_params[3]),
            max(0, oob_ori_roi[4] + padding_params[4]), max(0, vol_bound[5] - oob_ori_roi[5] + padding_params[5]),
        )
        ...
        pos3D_roi = dict(
            ...
            ori_roi=(
                max(0, cropping_params[0] + windows_clip[0]), max(0, cropping_params[0] + roi_shape[0] + windows_clip[1]),
                max(0, cropping_params[2] + windows_clip[2]), max(0, cropping_params[2] + roi_shape[1] + windows_clip[3]),
                max(0, cropping_params[4] + windows_clip[4]), max(0, cropping_params[4] + roi_shape[2] + windows_clip[5]),
            ),
            pred_roi=(
                max(0, padding_params[0] + windows_clip[0]), max(0, roi_shape[0] - padding_params[1] + windows_clip[1]),
                max(0, padding_params[2] + windows_clip[2]), max(0, roi_shape[1] - padding_params[3] + windows_clip[3]),
                max(0, padding_params[4] + windows_clip[4]), max(0, roi_shape[2] - padding_params[5] + windows_clip[5]),
            ))
        ...
        for box in boxes_3D:
            mapped_box = [
                max(box[0] - pos3D_roi['ori_roi'][0], 0), max(box[1] - pos3D_roi['ori_roi'][0], 0),
                max(box[2] - pos3D_roi['ori_roi'][2], 0), max(box[3] - pos3D_roi['ori_roi'][2], 0),
                max(box[4] - pos3D_roi['ori_roi'][4], 0), max(box[5] - pos3D_roi['ori_roi'][4], 0)
            ]
            mapped_boxes.append(mapped_box)
        # print(f'ori_roi: {pos3D_roi["ori_roi"]}, pred_roi: {pred_roi}, mapped_boxes: {mapped_boxes}, img3D_roi shape: {img3D_roi.shape}, gt3D_roi shape: {gt3D_roi.shape}')
...
def run_inference(args):
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if(args.skip_existing_pred and osp.exists(pred_path)):
            ...
            pred_3D_boxes = pred_3D_boxes[:20]
            ...
            sliding_window_list = pad_and_crop_with_sliding_window(image3D_full, gt3D_full, pred_3D_boxes, crop_transform, offset_mode=offset_mode)
            # import pdb; pdb.set_trace()
            for (image3D, gt3D, pos3D, mapped_boxes) in sliding_window_list:
                ...
                for idx, seg_mask in enumerate(seg_mask_list):
                    ...
                    # print(f'seg_mask_roi shape: {seg_mask_roi.shape}')
                    # print(f'pred3D_full_dict[idx] shape: {pred3D_full_dict[idx][..., ori_roi[0]:ori_roi[1], ori_roi[2]:ori_roi[3], ori_roi[4]:ori_roi[5]].shape}')
            ...
            # import pdb; pdb.set_trace()
            padding_params = sliding_window_list[-1][-2]["padding_params"]
            cropping_params = sliding_window_list[-1][-2]["cropping_params"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task_name', type=str, default='MICCAI_Challenge_2024_with_click_prompt_all_data')
    parser.add_argument('--save_name', type=str, default='./results/union_out_dice_use_gt_3Dbox_step_0_5_no_sliding.py')
    ...
    parser.add_argument('--Stage2_weights', type=str, default='/mnt/data1/yujunxuan/MICCAI_challenge_method/yolov5-SAM-Med3D/work_dir/Dataset131_MICCAI_challenge_small_v2/sam_model_dice_best.pth')
